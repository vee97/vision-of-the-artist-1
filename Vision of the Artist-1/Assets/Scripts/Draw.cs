...
// Credits to FuseVR for Tilt Brush tutorial
using System.Linq;
...
public class Draw : MonoBehaviour
{
    ...
    private List<GraphicsLineRenderer> lines = new List<GraphicsLineRenderer>();

    // keep track of the current line renderer
    //private LineRenderer currentLineRender;
    private GraphicsLineRenderer currentLineRender;
	...
	void AddNewLineRenderer()
	{
		...
		// set parent & position - try never mind to this
		//go.transform.parent = objectToTrackMovement.transform.parent;
		//go.transform.position = objectToTrackMovement.transform.position;

        // add Custom GraphicsLineRenderer to that game object
        go.AddComponent<MeshFilter>();
        go.AddComponent<MeshRenderer>();
        GraphicsLineRenderer goLineRenderer = go.AddComponent<GraphicsLineRenderer>();

        // set start and end width
        // goLineRenderer.startWidth = lineDefaultWidth;
        //goLineRenderer.endWidth = lineDefaultWidth;
        goLineRenderer.SetWidth(lineDefaultWidth);
        goLineRenderer.lmat = defaultLineMaterial;
    ...
    void UpdateLine()
    {
    	...
    	if(prevPointDistance != null && Mathf.Abs(Vector3.Distance(prevPointDistance, objectToTrackMovement.transform.position)) >= minDistanceBeforeNewPoint)
    	{
            ...
            currentLineRender.AddPoint(prevPointDistance, dir);
            positionCount++;
        }
    }
    ...
    void AddPoint(Vector3 position, Vector3 direction)
    {
        ...
        //commented out currentLineRender.AddPoint(objectToTrackMovement.transform.position);
        //currentLineRender.AddPoint(objectToTrackMovement.transform.position);
        //positionCount++;
    }
